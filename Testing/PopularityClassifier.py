############################################################
# Gradient Boosting Classifer for Popularity Prediction    #
...
def exportImportance(dir,kind,name):
        ...
        list_importance=[]
        i=0;
        while i < len(popularity_importance):
                list_importance.append([features[i],popularity_importance[i]])
                i = i + 1
...
def youtubePopular(dir,clf,imp):

        global popularity_importance
        popularity_importance = []
        
        metrics = []

        fp = open(os.path.join(dir,'youtube_popular.eva' ),'w')
        ...
        if imp == 1:
                ...
                popularity_importance = importance
                exportImportance(dir,0,"youtube_popular_all_old")
        elif imp == 0:
                print('\tAll(>14 days) AUC={0:0.2f}'.format(auc))
                fp.write('youtube_popular_all_old\t{0:0.4f}\n'.format(auc))
        ...
        if imp == 1:
                ...
                importance = clf.feature_importances_
                popularity_importance = importance
                exportImportance(dir,0,"youtube_popular_all_recent")
        elif imp==0:
                print('\tAll(<=14 days) AUC={0:0.2f}'.format(auc_recent))
                fp.write('youtube_popular_all_recent\t{0:0.4f}\n'.format(auc_recent))
                ...
                fp.write('youtube_popular_baseline_recent\t{0:0.4f}\n'.format(auc_base_recent))
                ...
                fp.write('youtube_popular_f1_score\t{0:0.4f}\n'.format(f1_average))
                ...
                plt.xlim([0.0,1.0])
                plt.ylim([0.0,1.05])
                plt.title('Youtube Popular')
                plt.legend(loc='best', fancybox='True', framealpha=0.5, fontsize=8);
                fig.savefig(os.path.join(dir,'popular_youtube_features.png'))
                plt.close(fig)
        elif imp == 2:
                # Calculating importance than >days(other)
                print('Calculating importance for <= days (other) using youtube Features ')
...
def twitterPopular(dir,clf,imp):

        global popularity_importance
        popularity_importance = []

        fp = open(os.path.join(dir,'twitter_popular.eva' ),'w')
        ...
        if imp == 1:
                ...
                importance = clf.feature_importances_
                popularity_importance = importance
                exportImportance(dir,1,"twitter_popular_all_old")
        elif imp==0:
                print('\tAll(>14 days) AUC={0:0.2f}'.format(auc))
                fp.write('twitter_popular_all_old\t{0:0.4f}\n'.format(auc))
                # Baseline features - other(>)
                X = tw_train_base
                Y = popular_train
                metrics = classify(clf,X,Y)
                ...
                f1_base_other = metrics[3]
                print('\tBaseline(>14 days) AUC={0:0.2f}'.format(auc_base))
                fp.write('twitter_popular_baseline_old\t{0:0.4f}\n'.format(auc_base))
        elif imp == 2:
                # Calculating importance than >days(other)
                print('Calculating importance for > days (other) using twitter Features ')
                importance = clf.feature_importances_
        ...
        if imp == 1:
                ...
                importance = clf.feature_importances_
                popularity_importance = importance
                exportImportance(dir,1,"twitter_popular_all_recent")
        elif imp == 0:
                print('\tAll(<=14 days) AUC={0:0.2f}'.format(auc_recent))
                fp.write('twitter_popular_all_recent\t{0:0.4f}\n'.format(auc_recent))
                # Baseline features - recent(<)
                ...
                f1_base_recent = metrics[3]
                print('\tBaseline(<=14 days) AUC={0:0.2f}'.format(auc_base_recent))
                fp.write('twitter_popular_baseline_recent\t{0:0.4f}\n'.format(auc_base_recent))

                # Calculating overall F1 score
                f1_average = (f1_all_other+f1_base_other+f1_all_recent+f1_base_recent)/4
                print('\t\tF1 score: {0:0.6f}'.format(f1_average))
                fp.write('twitter_popular_f1_score\t{0:0.4f}\n'.format(f1_average))
                ...
                plt.xlim([0.0,1.0])
                plt.ylim([0.0,1.05])
                plt.title('Twitter Popular')
                plt.legend(loc='best', fancybox='True', framealpha=0.5, fontsize=8);
                fig.savefig(os.path.join(dir,'popular_twitter_features.png'))
                plt.close(fig)
        elif imp == 2:
                # Calculating importance than >days(other)
                print('Calculating importance for <= days (other) using twitter Features ')
...
def bothPopular(dir,clf,imp):

        global popularity_importance
        popularity_importance = []

        fp = open(os.path.join(dir,'both_popular.eva' ),'w')
        ...
        #Predict Popularity using only Youtube features
        X = x_train_all
        Y = popular_train
        metrics = classify(clf,X,Y)
        precision = metrics[0]
        ...
        if imp ==1:
                ...
                importance = clf.feature_importances_
                popularity_importance = importance
                exportImportance(dir,2,"both_popular_all_old")
        elif imp == 0:
                print('\tAll(>14 days) AUC={0:0.2f}'.format(auc))
                fp.write('both_popular_all_old\t{0:0.4f}\n'.format(auc))
                ...
                f1_base_other = metrics[3]
                print('\tBaseline(>14 days) AUC={0:0.2f}'.format(auc_base))
                fp.write('both_popular_baseline_old\t{0:0.4f}\n'.format(auc_base))
        elif imp == 2:
                # Calculating importance than >days(other)
                print('Calculating importance for > days (other) using all Features ')
                importance = clf.feature_importances_
        ...
        if imp ==1:
                ...
                importance = clf.feature_importances_
                popularity_importance = importance
                exportImportance(dir,2,"both_popular_all_recent")
        elif imp == 0:
                print('\tAll(<=14 days) AUC={0:0.2f}'.format(auc_recent))
                fp.write('both_popular_all_recent\t{0:0.4f}\n'.format(auc_recent))
                ...
                f1_base_recent = metrics[3]
                print('\tBaseline(<=14 days) AUC={0:0.2f}'.format(auc_base_recent))
                fp.write('both_popular_baseline_recent\t{0:0.4f}\n'.format(auc_base_recent))

                # Calculating overall F1 score
                f1_average = (f1_all_other+f1_base_other+f1_all_recent+f1_base_recent)/4
                print('\t\tF1 score: {0:0.6f}'.format(f1_average))
                fp.write('both_popular_f1_score\t{0:0.4f}\n'.format(f1_average))
                ...
                plt.xlim([0.0,1.0])
                plt.ylim([0.0,1.05])
                plt.title('Both Popular')
                plt.legend(loc='best', fancybox='True', framealpha=0.5, fontsize=8);
                fig.savefig(os.path.join(dir,'popular_all_features.png'))
                plt.close(fig)
        elif imp == 2:
                # Calculating importance than <days(recent)
                print('\nCalculating importance for < days (recent) using all Features ')
